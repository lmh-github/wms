...
<div class="pageContent">
	<div class="pageFormContent" layoutH="97">
		<fieldset>
			...
			<dl>
				<dt>PO：</dt>
				<dd><input name="transfer.po" type="text" maxlength="20" size="20" value="${transfer.po }" /></dd>
			</dl>
			<dl>
				<dt>物流公司：</dt>
				<dd><input name="transfer.logisticName" type="text" maxlength="20" size="20" value="${transfer.logisticName }" /></dd>
			</dl>
			<dl>
				<dt>售达方：</dt>
				<dd><input name="transfer.tSale" type="text" maxlength="20" size="20" value="${transfer.tSale }" /></dd>
			</dl>
			<dl>
				<dt>送达方：</dt>
				<dd><input name="transfer.tSend" type="text" maxlength="20" size="20" value="${transfer.tSend }" /></dd>
			</dl>
			<dl>
				<dt>开票方：</dt>
				<dd><input name="transfer.tInvoice" type="text" maxlength="20" size="20" value="${transfer.tInvoice }" /></dd>
			</dl>
			<dl>
				<dt>调货类型：</dt>
				<dd>
		...
		<s:if test="transfer.status==1 || (transfer.status==3 && goodsList.size()==0)">
			<div class="panelBar">
				<ul class="toolBar">
					<li><a class="add" href="${ctx}/stock/transfer!inputGoodsNew.action?transferId=${transfer.transferId}" target="dialog" rel="dlg_transfer_goodsnew" mask="true" width="800" height="600"><span>添加调拨商品</span></a></li>
				</ul>
			</div>
		</s:if>
		<div class="tabs">
					...
					<table class="list" width="100%">
						...
						<tbody id="purRecvTbody">
							...
							<tr class="unitBox">
								...
								<s:if test="transfer.status==1">
								<td>
								<a title="确实要删除吗？" target="ajaxTodo" callback="goodsDeleteAjaxDone" href="${ctx}/stock/transfer!deleteGoods.action?goodsId=${id}&warehouseId=${transfer.warehouseId}" class="btnDel"></a>
								</td>
								</s:if>
							</tr>
							</s:iterator>
...
function goodsDeleteAjaxDone(json){
    ...
    if (json.statusCode == DWZ.statusCode.ok){
    	...
    	$.pdialog.reload('${ctx}/stock/transfer!inputManual.action?transfer.transferId=${transfer.transferId}',{dialogId:'dlg_transfer_manual'});
    }
}
</script>