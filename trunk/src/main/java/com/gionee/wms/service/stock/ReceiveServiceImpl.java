	...
	private void increaseStock(Receive receive) {
		try {
			...
			for (ReceiveSummary summary : recvSummaryList) {
				...
				StockRequest stockRequest = new StockRequest(summary.getWarehouseId(), summary.getSkuId(), stockType, summary.getQuantity(), bizType, receive.getOriginalCode());
