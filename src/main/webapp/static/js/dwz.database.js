	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				var addButId = $table.attr('id')?" id="+$table.attr('id'):"";
				if (addButTxt) {
					var $addBut = $('<div class="button"><div class="buttonContent"><button type="button"'+addButId+'>'+addButTxt+'</button></div></div>').insertBefore($table).find("button");
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					default:
						...
						if(field.onkeydown){
							readonly+=' onkeydown="'+field.onkeydown+'"';
						}
