...
<c:set var="rand"><%= java.lang.Math.round(java.lang.Math.random() * 100000) %>
</c:set>

<style type="text/css">#goodsTbody${rand} input[type=text] {
    width: 90%;
}</style>
<div class="pageContent">
    <form method="post" action="${ctx}/stock/morder!add.action?callbackType=closeCurrent&navTabId=tab_morder"
          class="pageForm required-validate" onsubmit="" id="form${rand}">
        <div class="pageFormContent" layoutH="56">
            <fieldset>
                <legend>基本信息</legend>
                <p>
                    <label>建单类型：</label>
                    <select name="order.billType">
                        <option value="赠品漏发">赠品漏发</option>
                        <option value="三包期限内质量问题补发">三包期限内质量问题补发</option>
                        <option value="系统BUG">系统BUG</option>
                        <option value="好评晒单">好评晒单</option>
                        <option value="京东赠品补发">京东赠品补发</option>
                        <option value="订单促销">订单促销</option>
                        <option value="售后客述安抚">售后客述安抚</option>
                    </select>
                </p>
                <p>
                    <label>需求平台：</label> <s:select name="order.platform"
                                                   list="@com.gionee.wms.common.WmsConstants$OrderSource@values()"
                                                   listKey="code" listValue="name"/>
                </p>
                <p>
                    <label>是否重开发票：</label> <select name="order.invoice">
                ...
                <dl class="nowrap">
                    <dt>建单原因：</dt>
                    <dd><textarea name="order.remark" cols="90" rows="2" maxlength="100" class="textInput valid"
                                  style="height: 45px;"></textarea></dd>
                </dl>
            </fieldset>
            <fieldset>
                <legend>基本信息</legend>
                <p>
                    <label>输入订单号：</label> <input type="text" size="30" value="" alt="输入订单号按回车带出其他信息" class="required"
                                                 id="orderCode${rand}"/>
                </p>
                <div class="divider"></div>
                <p>
                    <label>订单号：</label> <input type="text" name="order.salesOrder.orderCode" data-bind="orderCode"
                                               class="textInput required">
                </p>
                <p>
                    <label>订单来源：</label> <s:select data-bind="orderSource"
                                                   list="@com.gionee.wms.common.WmsConstants$OrderSourceGionee@values()"
                                                   name="order.salesOrder.orderSource" listKey="code" listValue="name"
                                                   headerKey="" headerValue="" disabled="false"/>
                </p>
                <p>
                    <label>订单状态：</label> <s:select data-bind="orderStatus"
                                                   list="@com.gionee.wms.common.WmsConstants$OrderStatus@values()"
                                                   name="order.salesOrder.orderStatus" listKey="code" listValue="name"
                                                   headerValue="" headerKey="" disabled="false"/>
                </p>
                <p>
                    <label>下单人：</label> <input type="text" name="order.salesOrder.orderUser" data-bind="orderUser"
                                               class="textInput">
                </p>
                <p>
                    <label>支付类型：</label> <s:select data-bind="paymentType"
                                                   list="@com.gionee.wms.common.WmsConstants$PaymentType@values()"
                                                   name="order.salesOrder.paymentType" listKey="code" listValue="name"
                                                   headerKey="" headerValue="" disabled="false"/>
                </p>
                <p>
                    <label>支付方式：</label> <input type="text" name="order.salesOrder.paymentName" data-bind="paymentName"
                                                class="textInput">
                </p>
                <p>
                    <label>配送方式：</label> <input type="text" name="order.salesOrder.shippingName"
                                                data-bind="shippingName" class="textInput">
                </p>
                <p>
                    <label>配送单号：</label> <input type="text" name="order.salesOrder.shippingNo" data-bind="shippingNo"
                                                class="textInput">
                </p>
                <p>
                    <label>收货人：</label> <input type="text" name="order.salesOrder.consignee" data-bind="consignee"
                                               class="textInput">
                </p>
                <p>
                    <label>手机号：</label> <input type="text" name="order.salesOrder.mobile" data-bind="mobile"
                                               class="textInput">
                </p>
                <p>
            ...
            <table class="list nowrap" width="100%" style="table-layout: fixed;">
                ...
                <tr>
                    <td>
                        <input class="textInput readonly" name="editSkuCode" bringBackName="sku.skuCode" type="text"
                               value="" lookupGroup="sku" readonly="readonly" style="width: 130px;"/>
                        <a class="btnLook" href="${ctx}/wares/sku!lookup.action" lookupGroup="sku"
                           width="1200">查找SKU</a>
                    </td>
