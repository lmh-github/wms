...
import com.gionee.wms.dao.DeliveryBatchDao;
...
//			}
		...
		try {
			...
			if(CollectionUtils.isNotEmpty(orderList)){
				...
				try {
					...
					paramsMap2.put("orderStatus",
							WmsConstants.OrderStatus.CANCELED.getCode());
					paramsMap2.put("opUser", ActionUtils.getLoginName() == null
		...
		try {
			...
			if(CollectionUtils.isNotEmpty(orderList)){
				...
				try {
					paramsMap2.put("orderIds", orderIds);
					paramsMap2.put("orderStatus",
							WmsConstants.OrderStatus.SHIPPED.getCode());
					paramsMap2.put("opUser", ActionUtils.getLoginName() == null
							? WmsConstants.DEFAULT_USERNAME_LOG
							: ActionUtils.getLoginName());
					paramsMap2.put("opTime", new Date());
			...
			for (DeliverySummary summary : goodsSummaryList) {
				StockRequest stockRequest = new StockRequest(summary.getWarehouseId(), summary.getSkuId(),
						StockType.STOCK_SALES, summary.getQuantity(), StockBizType.OUT_SALES, deliveryBatch
								.getBatchCode());
	...
	public void updateDeliveryIndivs(DeliveryGoods goods, List<IndivFlow> indivList) throws ServiceException {
		...
		for (IndivFlow currentIndiv : indivList) {
			...
			if (IndivStockStatus.STOCKOUT_HANDLING.getCode() == existingIndiv.getStockStatus()
					&& !delivery.getDeliveryCode().equals(existingIndiv.getOutCode())) {
		...
		try {
			...
			try {
				...
				paramsMap.put("orderStatus",
						WmsConstants.OrderStatus.PRINTED.getCode());
				paramsMap.put("opUser", ActionUtils.getLoginName() == null
						? WmsConstants.DEFAULT_USERNAME_LOG
						: ActionUtils.getLoginName());
...
//	@Override
//	public void confirmDelivery(Long warehouseId, String batchCode, String[] orderIds) {
//		// 获得所有配送方式
//		List<Shipping> validShippings = shippingService.getValidShippings();
//		Map<Long, Shipping> validShippingMap = Maps.newHashMap();
//		for (Shipping shipping : validShippings) {
...
//				criteria.put("outTime", new Date());	// 出库日期
...
//					StockRequest stockRequest = new StockRequest(warehouseId, skuId,
//					StockType.STOCK_OCCUPY, quantity, StockBizType.OUT_SALES, salesOrderVo.getOrderCode());
//					stockService.decreaseStock(stockRequest);
//				}
//				
...
//					if(null==shipping) {
//						throw new ServiceException(salesOrderVo.getOrderCode() + " 配送方式不存在, 无法订阅快递信息");
//					}
//					try {
