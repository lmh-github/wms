...
import com.gionee.wms.common.WmsConstants.TransferStatus;
...
public class TransferServiceImpl extends CommonServiceImpl implements TransferService {
	...
	public void confirmDelivery(Transfer transfer, String logisticNo, List<TransferGoods> goodsList) {
		...
		try {
			...
			if (null != indivList && indivList.size() > 0) {
				...
				criteria.put("stockStatus", WmsConstants.IndivStockStatus.OUT_WAREHOUSE.getCode());
			...
			for (TransferGoods goods : goodsList) {
				StockType stockType = StockType.STOCK_SALES;	// 变动的库存类型
				...
				StockRequest stockRequest = new StockRequest(transfer.getWarehouseId(), goods.getSkuId(), stockType, goods.getQuantity(), StockBizType.OUT_TRANSFER, transfer.getTransferId() + "");
			...
			transfer.setStatus(WmsConstants.TransferStatus.DELIVERYED.getCode());	// 设为已发货
			...
			if (poTransfer.getStatus() == TransferStatus.DELIVERYED.getCode()) {
	...
	public void confirmDeliverySf(Transfer transfer, List<TransferGoods> goodsList) {
		...
		try {
			...
			transfer.setStatus(WmsConstants.TransferStatus.DELIVERYED.getCode());	// 设为已发货
			Warehouse wh = warehouseService.getWarehouseByCode("SF");
			transfer.setTransferTo(String.valueOf(wh.getId()));
			// transfer.setLogisticNo(logisticNo); // 设置物流单号
			transfer.setShippingTime(nowTime);
			Transfer poTransfer = transferDao.getTransferById(transfer.getTransferId());
			if (poTransfer.getStatus() == TransferStatus.DELIVERYED.getCode()) {
	...
	private void increaseStock(Transfer transfer) {
		try {
			...
			for (TransferGoods good : goodsList) {
				StockType stockType = StockType.STOCK_SALES;
				StockBizType bizType = StockBizType.OUT_TRANSFER;
	...
	public void confirmBack(String[] skuCodes, String[] indivCodes, Integer[] waresStatuss, Integer[] indivEnableds, Long warehouseId) {
		...
		for (Map.Entry<String, Map<String, Integer>> entry : salesSkuStockMap.entrySet()) {
			for (Map.Entry<String, Integer> entry2 : entry.getValue().entrySet()) {
				StockRequest stockRequest = new StockRequest(warehouse.getId(), entry.getKey(), StockType.STOCK_SALES, entry2.getValue().intValue(), StockBizType.IN_BACK_TRANSFER, entry2.getKey());
	...
	public void cancelTransfer(Long transferId) {
		...
		try {
			...
			} else if (WmsConstants.TransferStatus.DELIVERYED.getCode() == transfer.getStatus() || WmsConstants.TransferStatus.CANCELED.getCode() == transfer.getStatus()) {
