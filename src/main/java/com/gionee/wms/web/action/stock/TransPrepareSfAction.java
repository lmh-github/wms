...
public class TransPrepareSfAction extends CrudActionSupport<Transfer> {
    ...
    private CommonAjaxResult dealCase(Transfer transfer, String indivCode) {
        ...
        for (Indiv indivTemp : indivList) {
            ...
            if (WmsConstants.IndivStockStatus.IN_WAREHOUSE.getCode() != indivTemp.getStockStatus()) {
                result.setOk(false);
            ...
            if (null != transfer.getTransType() && WmsConstants.TRANS_TYPE_DEFECTIVE == transfer.getTransType().intValue()) {
                // 次品单
                if (WmsConstants.IndivWaresStatus.NON_DEFECTIVE.getCode() == indivTemp.getWaresStatus()) {
                    result.setOk(false);
                    result.setMessage("次品调拨的物品必须为次品" + indivTemp.getIndivCode() + "非次品");
                    return result;
                }
            } else {
                // 良品单
                if (WmsConstants.IndivWaresStatus.DEFECTIVE.getCode() == indivTemp.getWaresStatus()) {
                    result.setOk(false);
                    result.setMessage("良品调拨的物品必须为良品" + indivTemp.getIndivCode() + "非良品");
                    return result;
                }
            }
        }
        for (TransferGoods goods : goodsList) {
            ...
            for (Indiv indiv : indivList) {
                if (indiv.getSkuCode().equals(goods.getSkuCode()) && WmsConstants.ENABLED_TRUE == goods.getIndivEnabled()) {
                    count = count + 1;
                    resultMap.clear();
                    ...
                    if (count + goods.getPreparedNum() > goods.getQuantity()) {
                        result.setOk(false);
                        result.setMessage("箱号所含sku：" + goods.getSkuCode() + "商品个数超出尚需配货的数量");
                        return result;
                    }
                }
            }
    ...
    public String list() throws Exception {
        ...
        ActionContext.getContext().getValueStack().set("warehouse", warehouse);
        return LIST;
    }
